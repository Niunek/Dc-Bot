...
async def lvl(ctx):
    ...
    name_usera = ctx.author.global_name
    avatar_img = load_image(ctx.author.display_avatar.url)
    avatar = Editor(avatar_img).resize((300,300)).circle_image()
    ...
    tlo.text((210, 860), str(name_usera),font = profil_czcionki_1, color="white")
    tlo.text((210, 920), str(tekst_poziomu),font = profil_czcionki_2, color="red")
